...
public class PasswordURLAuthenticatorTimerArduino  extends TimerTask {
	
	static HygroSample higroExt = new HygroSample();
	
    public void run() {
    	System.out.println("Hello World! - B.Exterior");
        Date date1 = new Date();
        
        if (higroExt.first==null){
        	System.out.println("First Day - B.Exterior");
    		...
    		higroExt.add(date1,timeS,tempS,humiS,stateS);
        }
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
				...
				double humi = Double.parseDouble(data[4].substring(5));
				String state = "NA";
				...
				if (sdf1.format(date2).equals(sdf1.format(date1))!=true){
		    		...
		    		double[] humiS=new double[288];
		    		String[] stateS=new String[288];    		
		    		higroExt.add(date2,timeS,tempS,humiS,stateS);
		            System.out.println("New Day");
		            higroExt.toCSV(higroExt.last, "Ext");
		            //higroExt.toHTML(higroExt.last, "Ext");
				}
				// Update last instant data
				higroExt.feed(higroExt.last, time, temp, humi, state);
            }
            in.close();
    ...
    public static void main(String[] args){
        ...
        //AnimatedTimeLineChart.launch(AnimatedTimeLineChart.class,args);
